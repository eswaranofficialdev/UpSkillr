  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 rounded-3xl shadow-xl">
        <div>
          <span className="text-xs font-bold uppercase tracking-wider text-indigo-500">Current Career Goal</span>
          <h1 className="text-3xl font-black mt-1">{profile.goal}</h1>
          <p className="text-slate-500 text-sm mt-1">Experience: {profile.experience} • Goal: {profile.weeklyHours}/week</p>
        </div>
        <div className="flex items-center gap-3">
          <Link to="/roadmap" className="px-6 py-3 rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-sm shadow-lg flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-3xl">
          <div className="flex items-center justify-between mb-4"><span className="text-slate-500 text-xs font-bold uppercase">Est. Duration</span><Clock className="w-5 h-5 text-indigo-500"/></div>
          <div className="text-3xl font-black mb-1">{roadmap.estimatedDuration}</div>
          <p className="text-xs text-slate-500">Based on {profile.weeklyHours}</p>
        </div>
      </div>
    </div>
  );
