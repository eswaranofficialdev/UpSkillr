...
export default function RoadmapPage() {
  ...
  // Safety check if roadmap hasn't loaded yet
  if (!roadmap || !roadmap.phases) {
    return (
      <div className="max-w-5xl mx-auto px-4 py-20 text-center space-y-4">
        <h2 className="text-2xl font-black">No roadmap found!</h2>
        <p className="text-slate-500 text-sm">Please generate your personalized roadmap from onboarding.</p>
        <div>
          <Link to="/onboarding" className="inline-block px-6 py-3 rounded-xl bg-emerald-600 text-white font-bold text-sm">
            Generate Roadmap
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8 sm:space-y-12 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <span className="text-xs font-bold uppercase tracking-wider text-emerald-500">Target Role</span>
          <h1 className="text-2xl sm:text-4xl font-black mt-1">{roadmap.careerTitle || 'Developer'} Path</h1>
        </div>
        <div className="flex items-center gap-2 text-xs font-semibold px-3.5 py-2 rounded-xl bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 self-start sm:self-auto">
          <Clock className="w-4 h-4"/> Est. Duration: {roadmap.estimatedDuration || '3 Months'}
        </div>
      </div>

      {/* Timeline Container */}
      <div className="space-y-10 relative before:absolute before:inset-0 before:left-3.5 sm:before:left-6 before:w-0.5 before:bg-gradient-to-b before:from-emerald-500 before:via-teal-500 before:to-slate-300 dark:before:to-slate-800">
        {roadmap.phases.map((phase, pIdx) => (
          <div key={phase.phaseNumber || pIdx} className="relative pl-10 sm:pl-16">
            <div className="absolute left-3.5 sm:left-6 -translate-x-1/2 top-1.5 w-4 h-4 rounded-full bg-emerald-500 ring-4 ring-slate-100 dark:ring-slate-950 z-10"></div>
            
            <h3 className="text-base sm:text-lg font-bold mb-4 text-emerald-600 dark:text-emerald-400 flex items-center gap-2">
                ...
                <div 
                  ...
                  className={`node-card p-4 sm:p-5 rounded-2xl border cursor-pointer transition-all duration-200 hover:-translate-y-1 active:translate-y-0 flex flex-col justify-between ${
                    skill.completed 
                      ...
                      : 'bg-white dark:bg-slate-900/80 border-slate-200 dark:border-slate-800 hover:border-emerald-500/50 shadow-sm'
                  }`}
                >
                  <div>
